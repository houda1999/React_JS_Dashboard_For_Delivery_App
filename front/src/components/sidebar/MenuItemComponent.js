...
const useStyles = createUseStyles({
    ...
    leftBar: {
        borderLeft: ({ theme, level }) =>
            level > 1 ? 'none' : `3px solid ${theme.color.orange}`
    },
    title: {
        fontSize: 16,
        lineHeight: '20px',
        letterSpacing: '0.2px',
        color: ({ theme, isActive }) => (isActive ? theme.color.orange : theme.color.grayishBlue),
        marginLeft: 24
    }
});

function MenuItemComponent({ children,iconItem:Icon , id, items = [], level = 1, onClick, title }) {
    ...
    const iconColor = isActive ? theme.color.orange : theme.color.grayishBlue2;
    ...
    return (
        <Column key={id} className={classNameColumn}>
            ...
            <FontAwesomeIcon icon={Icon} color={iconColor}  opacity={opacity} />
